...
class BaseModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        ...
        self.generator = WanDiffusionWrapper(checkpoint_path=args.checkpoint_path, **getattr(args, "model_kwargs", {}), is_causal=True)
        self.generator.model.requires_grad_(True)

        self.real_score = WanDiffusionWrapper(checkpoint_path=args.checkpoint_path, model_name=self.real_model_name, is_causal=False)
        self.real_score.model.requires_grad_(False)

        self.fake_score = WanDiffusionWrapper(checkpoint_path=args.checkpoint_path, model_name=self.fake_model_name, is_causal=False)
        self.fake_score.model.requires_grad_(True)

        self.text_encoder = WanTextEncoder(checkpoint_path=args.checkpoint_path)
        self.text_encoder.requires_grad_(False)

        self.vae = WanVAEWrapper(checkpoint_path=args.checkpoint_path)
...
class BaseRLModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        self.inferencer = VideoVLMRewardInference(f"{args.checkpoint_path}/VideoReward")
...
class RewardForcingModel(BaseRLModel):
    def __init__(self, args, device):
        super().__init__(args, device)
        self.denoising_loss_func = get_denoising_loss(args.denoising_loss_type)()
