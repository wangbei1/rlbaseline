...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, checkpoint_path) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(f"{checkpoint_path}/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, checkpoint_path):
        ...
        self.model = _video_vae(
            pretrained_path=f"{checkpoint_path}/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"{checkpoint_path}/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(f"{checkpoint_path}/{model_name}/")
        self.model.eval()
